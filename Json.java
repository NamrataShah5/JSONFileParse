...
public class Json {
	...
	public static void main(String[] args) {
		...
		try {
			URL url = new URL("http://www.json-generator.com/api/json/get/ceCPjXHhsO?indent=2");
			...
			if (responsecode != 200)
				...
				// JSONParser reads the data from string object and breaks each data into key
				// value pairs
							...
							for (int i = 0; i < cyc.get(s).size(); i++) {
								...
								if (!poss.isEmpty()) {
									if (!poss.contains(childs)) {
										...
										obj.put("root_id", ss);
										if (childs.charAt(childs.length() - 2) != ss.charAt(0)) {
											valid.add(obj);
										} else {
											inv.add(obj);
										}
	...
	public static ArrayList<Integer> DFS(HashMap<Integer, ArrayList> cyc, int adj, boolean[] visited,
			ArrayList<Integer> path, boolean iff) {
